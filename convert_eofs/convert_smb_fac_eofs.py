...
def convert_smb_fac_eofs(EOF_FILE, LMAX, MMAX=None, RAD=0,
    ...
    regex_pattern = r'(GSFC|GEMB|IMAU)(.*?)_EOF_SMB_FAC_(.*?)(sep_)?(ais|gris)(.*?).nc$'
    MODEL,AUX1,AUX2,SEP,REGION,AUX3 = re.findall(regex_pattern, EOF_FILE.name).pop()
    ...
    output_to_netCDF4(OUTPUT_FILE, output,
        grid=GRID, model=MODEL, region=REGION)
...
def main():
    ...
    # run program
    convert_smb_fac_eofs(args.infile, args.lmax,
        MMAX=args.mmax,
        RAD=args.radius,
        MASKS=args.mask,
        ATL15=args.atl15,
