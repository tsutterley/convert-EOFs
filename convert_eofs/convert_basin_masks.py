...
def convert_basin_masks(INPUT_FILE, LMAX, MMAX=None,
    RAD=0, ATL15=None, BUFFER=800e3, SIGMA=0,
    TOLERANCE=1, MODE=0o775):
    ...
    if (SIGMA > 0):
        ...
        for band in range(nband):
            ...
            temp = scipy.ndimage.gaussian_filter(temp.astype(np.float64),
...
class mask(gravtk.harmonics):
    ...
    def to_netCDF4(self, filename, **kwargs):
        ...
        # spherical harmonics
        nc['clm'] = fileID.createVariable('clm', 'd', ('lm','band',))
        nc['slm'] = fileID.createVariable('slm', 'd', ('lm','band',))
        # bands
...
def arguments():
    ...
    parser.add_argument('infile',
        type=pathlib.Path,
        help='Input drainage basin file')
...
def main():
    ...
    # run program
    convert_basin_masks(args.infile, args.lmax,
        MMAX=args.mmax,
        RAD=args.radius,
        ATL15=args.atl15,
        SIGMA=args.sigma,
