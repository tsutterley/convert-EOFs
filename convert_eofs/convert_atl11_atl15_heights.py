...
def convert_atl11_atl15_heights(
    ...
    struct = dict(dimensions=("time", "y", "x"), variables={})
    ...
    for var in fields:
        ...
        if CLM_FILE.exists():
            ...
            Ylms = gravtk.harmonics().from_netCDF4(filename=CLM_FILE)
            # for each time step
            for n in range(nt):
        ...
        # fix error variables to be positive to account for ringing
        if var in ("rand_e", "sys_e"):
            output[var] = np.abs(output[var])
